...
Ces valeurs sont des tokens. Rien ne les recalcule ailleurs.

| Valeur | Token | Pixels |
|---|---|---|
| Bandeau haut | `--cadre-haut` | `180` |
| Colonne latérale | `--cadre-colonne` | `320` |
| Bandeau bas | `--cadre-bas` | `180` |
| Scène, origine X | `--scene-x` | `320` |
| Scène, origine Y | `--scene-y` | `180` |
| Scène, largeur | `--scene-largeur` | `1280` |
| Scène, hauteur | `--scene-hauteur` | `720` |

Les colonnes et les bandeaux ne sont pas des valeurs indépendantes : ils bordent la scène.
`colonne + scène + colonne` fait 1920, `haut + scène + bas` fait 1080. Un test le vérifie,
pour qu'un ajustement de la scène ne laisse jamais une bande orpheline.
